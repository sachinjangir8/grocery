...
import { ChevronLeft, ChevronRight, Milk, Wheat, Cookie, Flame, Coffee, Heart, Home, Grid } from "lucide-react";
...
const categories = [
    ...
    { name: "All", icon: <Grid />, color: "bg-green-100 text-green-700" },
]
                ...
                >
                    {categories.map((cat, index) => (
                        <motion.div
                            ...
                            viewport={{ once: false }}
                            className={`min-w-40 h-[180px] ${cat.color} rounded-2xl p-6 flex flex-col items-center justify-center text-center gap-4 shadow-sm hover:shadow-md transition-all cursor-pointer border-2 ${currentCategory === cat.name ? 'border-emerald-500 scale-105' : 'border-white/50'} flex-shrink-0`}
                            onClick={() => {
