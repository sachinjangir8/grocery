...
import { ArrowLeft, CreditCard, MapPin, Package, ChevronDown, ChevronUp, Loader2, ShoppingBag, Navigation } from 'lucide-react';
    ...
    >
      ...
      <div className="p-4 border-t border-gray-50 flex justify-between items-center text-sm">
        <div className="flex flex-col gap-2">
            ...
            {(order.status === 'outForDelivery' || order.status === 'arrived') && (
                <button 
                    onClick={() => window.location.href = `/user/track-order/${order._id}`}
                    className="bg-green-600 text-white px-4 py-1.5 rounded-lg font-bold shadow-sm hover:bg-green-700 transition-all flex items-center gap-2"
                >
